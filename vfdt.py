...
def main():
    ...
    rows = 1000  # if more than 4521, it revert back to 4521
    ...
    # generate unique values for each feature for use in VFDT
    features = title[:-1]
    feature_values = {f:None for f in features}  # unique values in each feature
    bins = 0 # bins for discretization
    for f in features:
    ...
    for f in features:
        if (df[f].dtype == np.float64 or df[f].dtype == np.int64):
            ...
            # pd.cut to discretize continous feature values
    ...
    # heoffding bound parameter, with 1 - delta probability
    # the true mean is at least r - gamma
    delta = 0.01
    nmin = 5  # vfdt parameter, test split if new sample size > nmin
